...
function withDefaults(wallet) {
  return {
    ...
    availableTaprootSats:
      typeof wallet.availableTaprootSats === "number"
        ? wallet.availableTaprootSats
        : typeof wallet.balanceSats === "number"
        ? wallet.balanceSats
        : 0,
    taprootAddresses: Array.isArray(wallet.taprootAddresses)
...
function toJSON(wallets) {
  return wallets.map((w) => ({
    ...
    availableTaprootSats:
      typeof w.availableTaprootSats === "number"
        ? w.availableTaprootSats
        : typeof w.balanceSats === "number"
        ? w.balanceSats
        : 0,
    taprootAddresses: Array.isArray(w.taprootAddresses)
      ? w.taprootAddresses
      : w.p2tr
      ? [w.p2tr]
      : [],
...
function createWalletObject({ key, params }, pair) {
  ...
  return {
    ...
    p2tr: p2tr.address || "",
    taprootAddresses: p2tr.address ? [p2tr.address] : [],
    createdAt: now,
    updatedAt: now,
    note: "",
    balanceSats: 0,
    availableTaprootSats: 0,
    history: [],
...
export function recordWalletEvent({
  ...
  let taprootAddresses = Array.isArray(wallets[idx].taprootAddresses)
    ? [...wallets[idx].taprootAddresses]
    : wallets[idx].p2tr
    ? [wallets[idx].p2tr]
    : [];
  if (
    type === "receive" &&
    typeof relatedAddress === "string" &&
    /^(tb1p|bc1p)/i.test(relatedAddress) &&
    !taprootAddresses.includes(relatedAddress)
  ) {
    taprootAddresses.push(relatedAddress);
  }
  wallets[idx] = {
    ...wallets[idx],
    balanceSats: updatedBalance,
    availableTaprootSats: updatedBalance,
...
export function setWalletBalance(
  ...
  const idx = wallets.findIndex((w) => w.id === walletId);
  if (idx === -1) return null;
  const target = withDefaults(wallets[idx]);
  ...
  if (source === "sync") {
    const confirmed = Math.max(0, Math.trunc(Number(newBalanceSats) || 0));
    const pending = Math.max(0, Math.trunc(Number(options.pendingSats) || 0));
    const now = new Date().toISOString();
    wallets[idx] = {
      ...target,
      balanceSats: confirmed + pending,
      availableTaprootSats: confirmed,
