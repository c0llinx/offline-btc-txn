...
function random32() {
  ...
  for (let i = 0; i < 32; i++) fallback[i] = Math.floor(Math.random() * 256) & 0xff;
  return fallback;
}

export function generateBurnedInternalKey() {
  for (let i = 0; i < 128; i++) {
    const scalar = random32();
    const point = ecc.pointFromScalar(scalar, true);
    if (point) {
      return Buffer.from(point.slice(1, 33)); // x-only
    }
  }
...
export function buildClaimScript(R_xonly, h32) {
  ...
  if (!R_xonly || R_xonly.length === 0) {
    return bitcoin.script.compile([OPS.OP_SHA256, h, OPS.OP_EQUAL]);
  }
...
export function buildClaimRefundTaproot({
  R_xonly,
  S_xonly,
  h32,
  H_exp,
  ...
  const scriptTree = [{ output: claim }, { output: refund }];
  const requiresSignature = !!(R_xonly && R_xonly.length);
